...
print("Generating sessions and transactions (this will take a while)...")
...
# Save transactions
print("Saving transactions.json...")
...
for i in range(0, len(sessions), CHUNK_SIZE):
    ...
    print(f"  Saving sessions_{chunk_num}.json...")
    with open(f"sessions_{chunk_num}.json", "w") as f:
...
- transactions.json ({len(transactions):,} transactions)
- sessions_*.json ({len(sessions):,} sessions across multiple files)

Statistics:
- Sessions: {len(sessions):,} (target: {NUM_SESSIONS:,})
- Transactions: {len(transactions):,} (target: {NUM_TRANSACTIONS:,})
- Remaining product inventory: {sum(p['current_stock'] for p in inventory.products.values()):,} units
================================================================================
""")